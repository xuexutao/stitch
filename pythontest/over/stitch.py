...
# img_a  :  大图
# img_b  :  小图
def find_homography_no_record(img_a, img_b, visualize_matches=False):
    orb = cv2.ORB_create(nfeatures=10000,
                            scaleFactor=1.3,      # 金字塔缩放因子(原1.2)
                            edgeThreshold=15,     # 边界阈值
                            patchSize=31          # 描述符区域大小
...
# input five images   
def stitch(retts : list, first, MS, padding_color = False, record_time = False ):
    ...
    for i, img_b in enumerate(img_bs):
        ...
        if first == 1:
            ...
            if record_time:
                ...
                M = find_homography_no_record(gray_a, gray_b,  visualize_matches = True)
...
def output_button(left_up_path, bigFov_path, right_up_path, left_bottom_path, right_bottom_path):
    zhanshi = False
    psnr = False
    global first
    video_paths = [
    ...
    while True:
        ...
        if not choose:
            padding_color = False
# 第四个参数padding_color， 默认为False
        stitched_frame, _MS = stitch(frames, first, MS, False, record_times)
        if choose:
            MS = _MS
        if psnr:
...
#     parser.add_argument("--psnr", action='store_false', default=False)
...
#         stitched_frame, _MS = stitch(frames, first, MS, padding_color, record_times)
#         if choose:
#             MS = _MS
#         if args.psnr:
#             psnr = calculate_psnr(stitched_frame, cv2.resize(frames[1], None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC))
